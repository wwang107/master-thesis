...
class PanopticDataset(Dataset):
    ...
    def __init__(self, cfg,
        ...
        self.seq_names = TRAIN_LIST if is_train == True else VAL_LIST
        ...
        for seq_name in self.seq_names:
            ...
            # Load skeleton
            skel_json_paths = sorted(list(rootDir.joinpath(
                seq_name, 'hdPose3d_stage1_coco19').glob('*.json')))
            
            start_frame = self.num_frames_in_subseq if is_train else len(skel_json_paths)//2
    ...
    def __getitem__(self, idx):
        ...
        camera_ids = random.sample(TRAINING_CAMERA_ID, self.num_view) if self.is_train else VALIDATION_CAMERA_ID

        hm = np.zeros((
            self.num_directional_keypoint,
            self.cfg.DATASET.OUTPUT_SIZE[1], self.cfg.DATASET.OUTPUT_SIZE[0],
            self.num_view,
            self.num_frames_in_subseq), dtype=np.float32)
        img = np.zeros((
        ...
        for f, frame3d in enumerate(subseq_pose3d):
            for k, cam_id in enumerate(camera_ids):
                ...
                num_person[f] = p+1
                keypoint2d[0:num_person[f], 0:17, :, k, f] = pose2d[0:num_person[f],
                                                                 :, 0:17, k, f].transpose((0, 2, 1))
                keypoint2d[0:num_person[f], 17:, :, k, f] = \
